...
TYPE2 = ['subclasses','intersection','bbbb','ffff','aaaa','dddd','cccc','has']
...
def formatType1(sentence, n, words):
    ...
    if len(words) >4:
        if ('dluli' in words[4] or 'phezu' in words[4]):
            start = end+1
            end = start+len(words[4])
            lst.append(['a',start,end])
        
        elif (words[-1]=='ncamashi'):
            start = sentence.find('ncamashi')
...
def formatType2(sentence, noun2, words):
    ...
    if (noun2=='has'): #The verb is at the end of the "na" or whatever its coalscene was
        
        start = end + 1
        length = len(words[1])
        end = words[1].find('n')+2+start
    else:
        noun2 = noun2[1:]
        length = len(noun2)
