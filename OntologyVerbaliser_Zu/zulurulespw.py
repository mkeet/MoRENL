...
from NumberContextControl import NumberContextControl
...
# Renovation of exists_zu. Adding verbalisation of numbers
# simple existential quantification
# modified cf zulurules to handle also vowel-commencing vroots
def exists_modified(sub,op,super,cardinal,numType):
    nc1m = find_nc(sub)
    nc2m = find_nc(super)
    pl = plural_zu(sub,nc1m)
    pl2 = plural_zu(super,nc2m)
    ncp2 = look_ncp(nc2m)
    ncp = look_ncp(nc1m)
    qca = look_qca(ncp)
    rc = look_relc(ncp2)  
    rt = find_rt(op)
    verbalisedNumber = verbalise_number(cardinal,numType,pl2)
    ...
    if numType == "EXACTLY":
        return qca + ' ' + pl + ' ' + conjugrt + 'a' + ' ' + pl2 + ' ' +verbalisedNumber +' '+cardinalityType[numType]
    else:
        adverb = rc + cardinalityType[numType]
        return qca + ' ' + pl + ' ' + conjugrt + 'a' + ' ' + pl2 + ' ' +  adverb +' '+ verbalisedNumber
   
    
"""
#Data_instances
def data_instance(sub,op,cardinal):
    nc1m = find_nc(sub)
    nc2m = find_nc(op)
    pl2 = plural_zu(op,nc2m)
    sc = look_sc(nc1m)
    conjp = vowel_coal('na',pl2)
    verbalisedNumber = verbalise_number(cardinal,"EXACTLY",pl2)
    
    print(sub + " " + sc+ " " + conjp + " " + verbalisedNumber)
    return sub + " " + sc+ conjp + " " + verbalisedNumber
"""
#Method for verbalising data properties
def data_properties(sub,op,super):
    nc1m = find_nc(sub)
    sc = look_sc(nc1m)
    conjp = vowel_coal("na",super)
...
cardinalityType = {"MIN": "ngaphezu","MAX": "ngadluli","EXACTLY": "ncamashi"}
# akhiwe
# renovation of const_a, addition of verbalisation of numbers
def akhiwe(whole,part,cardinal,type):
    nc1m = find_nc(whole)
    nc2m = find_nc(part)
    pl = plural_zu(whole,nc1m)
    pl2 = plural_zu(part,nc2m)
    ncp = look_ncp(nc1m)
    qca = look_qca(ncp)
    ncp2 = look_ncp(nc2m)
    rc = look_relc(ncp2)  
    ...
    conjverb = sc_vowel_vroot('akhiwe',ncp)
    ofpart = vowel_coal('nga',pl2)
    verbalisedNumber = verbalise_number(cardinal,type,pl2)
    if type == "EXACTLY":
        return qca + ' ' + pl + ' ' + conjverb + ' ' + ofpart +' '+verbalisedNumber+' '+cardinalityType[type]
    else:
        adverb = rc + cardinalityType[type]
        return qca + ' ' + pl + ' ' + conjverb + ' ' + ofpart +' '+ adverb +' '+verbalisedNumber
        
#Method for verbalising a number 
def verbalise_number(cardinal,type,noun):
    numberContextControl = NumberContextControl(cardinal,noun,"C")
    if type == "EXACTLY":
        #Qualifying the noun
        return numberContextControl.verbaliseWith()
    else:
        if (cardinal>= 1 and cardinal <=5):
            verbalisedNumber = numberContextControl.verbaliseWith()
        else:
            verbalisedNumber = numberContextControl.verbaliseWithout()
        
        return vowel_coal('kwa',verbalisedNumber)
